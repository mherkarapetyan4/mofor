...
import MeasureData from "components/PregnancyMeasuring/Measuring/MeasureData";
...
class Measuring extends PureComponent {
    ...
    renderData = () => {
        const { data, theme, overdue, onClick } = this.props;
        if (overdue) {
            return (
                <Notification>
                    <Icon>
        ...
        return (
            <List
                data={data}
                renderItem={(item, i) => (
                    <MeasureData key={i} data={item} onClick={onClick} />
                )}
                rigid
            />
        );
