...
import { LK_MENU_ELEMENTS } from "config/menu";
import { eventActionHandler } from "actions/pregnancy";
import { pregnancyPath } from "config/paths";
import Actions from "containers/Header/Actions";
import { Desktop } from "wrappers/responsive";
import CalendarInPopup from "components/PregnancyEvents/CalendarInPopup";
...
@withTheme
@connect()
class PregnancyEvents extends PureComponent {
    static propTypes = {
    ...
    planningHandler = () => {
        const { planning, planningType } = this.props.data;
        if (planning.allowed) {
            if (planningType === "CALENDAR") {
                this.props.history.push(LK_MENU_ELEMENTS.CALENDAR_PAGE.path);
            } else if (planningType === "EMIAS") {
                this.props.history.push(LK_MENU_ELEMENTS.DOCTOR_PAGE.path);
            }
        } else {
            this.notAllowed(planning.message);
        }
    };
        ...
        if (action.allowed) {
            dispatch(
                eventActionHandler(
                    { id },
                    key === "unconfirmation"
                        ? pregnancyPath.EVENT_UNCONFIRMATION
                        : pregnancyPath.EVENT_REJECTION,
                ),
            );
        } else {
